import os, re, json, aiohttp, urllib.parse, threading
...
async def generate_fields(idea: str):
    ...
    return idea[:30].title(), "Pop, emotional, 130 BPM", f"[Verse 1]\n{idea}\n[Chorus]\n{idea}"
...
async def call_suno(title, style, lyrics):
    ...
    async with aiohttp.ClientSession() as s:
        async with s.get(url, timeout=150) as r:
            try:
                data = await r.json()
...
async def on_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "await_title":
        ctx.user_data['title'] = text[:80]; ctx.user_data['state'] = "await_style"
        await update.message.reply_text(f"✅ Title: *{text}*\nNow send Style:", parse_mode="Markdown"); return
    elif state == "await_style":
        ctx.user_data['style'] = text[:120]; ctx.user_data['state'] = "await_lyrics"
        await update.message.reply_text(f"✅ Style: *{text}*\nNow send Lyrics:", parse_mode="Markdown"); return
    elif state == "await_lyrics":
        ...
        ctx.user_data['lyrics'] = text[:3000]; ctx.user_data['state'] = None
        t,s,_ = await generate_fields(text[:100])
        ctx.user_data['title']=t; ctx.user_data['style']=s
        await update.message.reply_text(f"*Song Title:*\n{t}\n\n*Style Prompt:*\n{s}\n\n*Full Lyrics:*\n{text[:3500]}", parse_mode="Markdown", reply_markup=continue_kb()); return
    elif state in ["edit_title","edit_style","edit_lyrics"]:
        key = state.split("_")[1]; ctx.user_data[key] = text[:3000]; ctx.user_data['state']=None
        await update.message.reply_text("Updated ✅", reply_markup=continue_kb()); return
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "mode_ai":
        ...
        await q.edit_message_text("✍️ Send your own lyrics now:", parse_mode="Markdown")
    elif data == "mode_full_custom":
        ctx.user_data['state']="await_title"
        await q.edit_message_text("1️⃣ Send Title:", parse_mode="Markdown")
    elif data == "continue":
        ...
        if track and track.get('musicFile'):
            if track.get('coverImage'):
                try: await ctx.bot.send_photo(q.message.chat_id, track['coverImage'], caption=f"🎵 {title}")
                except: pass
            await ctx.bot.send_audio(q.message.chat_id, track['musicFile'], title=title, performer="Suno AI")
        else:
            await ctx.bot.send_message(q.message.chat_id, "❌ Suno failed, try again")
...
def main():
    # Start Flask in background for Render Web Service
    ...
    print("✅ Suno Bot Web Service Live")
    app.run_polling()
